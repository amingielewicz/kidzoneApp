...
/**
 * Preferencje powiadomień push użytkownika.
 *
 * Pola odpowiadają mapie `notificationPreferences` w `users/{uid}`:
 *  - newReviewOnMyPlace: ktoś wystawił opinię na moim miejscu
 *  - newPlaceNearby: nowe miejsce w okolicy (future)
 *  - weeklyDigest: podsumowanie tygodniowe (future)
 *
 * Domyślnie wszystkie włączone (true). User może wyłączyć poszczególne typy.
 */
...
/**
 * Dialog z ustawieniami powiadomień push.
 */
@Composable
fun NotificationPreferencesDialog(
    ...
    var prefs by rememberSaveable(currentPrefs) { mutableStateOf(currentPrefs) }
