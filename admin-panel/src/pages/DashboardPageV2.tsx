...
export function DashboardPage() {
  ...
  const fetchDashboard = useCallback(async () => {
    setLoading(true);
    try {
      const [placesC, reviewsC, usersC, changeRequestsC, usersSnap, placesSnap, reviewsSnap] =
        await Promise.all([
          getCountFromServer(collection(db, 'places')),
          getCountFromServer(collection(db, 'reviews')),
          getCountFromServer(collection(db, 'users')),
          getCountFromServer(
            query(collection(db, 'place_change_requests'), where('status', '==', 'pending')),
          ),
      ...
      setStats({
        ...
        pendingChangeRequests: changeRequestsC.data().count,
      });
    } catch (err) {
  ...
  const statCards = [
    {
      label: 'Miejsca',
      value: stats.places,
      icon: <PlaceIcon sx={{ fontSize: 36, color: '#1976D2' }} />,
      border: '#1976D2',
    },
    {
      label: 'Opinie',
      value: stats.reviews,
      icon: <ReviewsIcon sx={{ fontSize: 36, color: '#388E3C' }} />,
      border: '#388E3C',
    },
    {
      label: 'Użytkownicy',
    ...
    },
    {
      ...
      icon:
        stats.pendingChangeRequests > 0 ? (
          <NotificationsIcon sx={{ fontSize: 36, color: '#D32F2F' }} />
        ) : null,
      border: stats.pendingChangeRequests > 0 ? '#D32F2F' : '#BDBDBD',
    },
    {
      ...
      border: stats.pendingReports > 0 ? '#D32F2F' : '#BDBDBD',
    },
  ];
  ...
  return (
    <Box>
            ...
            <List dense disablePadding>
              {recentUsers.map((u) => (
                <ListItem key={u.id} disableGutters>
                  ...
                  <ListItemText primary={u.name || u.email} secondary={formatDate(u.createdAtMillis)} />
                </ListItem>
              ))}
            ...
            <List dense disablePadding>
              {recentPlaces.map((p) => (
                <ListItem key={p.id} disableGutters>
                  ...
                  <ListItemText primary={p.name} secondary={formatDate(p.createdAtMillis)} />
              ...
              {recentReviews.map((r) => (
                <ListItem key={r.id} disableGutters>
                  <ListItemText
                    ...
                    secondary={r.comment || '(bez komentarza)'}
                  />
                </ListItem>
              ))}
      ...
      <Paper sx={{ p: 2, mt: 3 }}>
        <Box display="flex" alignItems="center" gap={1} mb={2}>
          {newReportsCount > 0 && <ReportIcon color="error" />}
          <Typography variant="h6" fontWeight={600}>
            Nowe zgłoszenia
          </Typography>
          <Chip label={newReportsCount} size="small" color="error" />
          <Box sx={{ flexGrow: 1 }} />
          <Button size="small" onClick={handleClearNewReports} disabled={newReportsCount === 0}>
            Wyczyść
          </Button>
        </Box>
          ...
          <List dense disablePadding>
            {recentReports.map((r) => (
              <ListItemButton
                key={`${r.type}-${r.id}`}
                onClick={() =>
                  navigate(`/reports?tab=${r.type === 'place' ? 0 : r.type === 'review' ? 1 : 2}`)
                }
              ...
              >
                <ListItemAvatar>
                  <Avatar
                    sx={{
                      ...
                      height: 32,
                      bgcolor:
                        r.type === 'place' ? '#ffebee' : r.type === 'review' ? '#fff3e0' : '#fce4ec',
                    }}
                  >
                ...
                <ListItemText
                  ...
                  secondary={`${formatDate(r.createdAtMillis)}${r.comment ? ` — ${r.comment}` : ''}`}
                />
              </ListItemButton>
            ))}
          </List>
