...
// --- HTTP Endpoint: Admin rate limit check (callable) ---
// Firestore Rules already enforce 1 report per user per target.
// This CF adds time-based rate limiting for write-heavy operations.
export const checkRateLimit = onRequest(
  ...
  async (req, res) => {
    ...
    try {
      const decoded = await admin.auth().verifyIdToken(idToken);
      const uid = decoded.uid;
      const action = req.query.action as string; // e.g. "addPlace", "addReview", "report"
      ...
      if (!config) {
        res.status(200).json({allowed: true}); // Unknown action = no limit
        return;
      }
      ...
      const count = recentDocs.size;
      if (count >= config.maxPerHour) {
        res.status(429).json({
          allowed: false,
          reason: `Przekroczono limit ${config.maxPerHour} akcji na godzinę. Spróbuj później.`,
          retryAfterMinutes: 60,
        });
      } else {
        res.status(200).json({allowed: true, remaining: config.maxPerHour - count});
      }
    } catch (err) {
